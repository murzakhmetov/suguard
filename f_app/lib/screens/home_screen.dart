...
import 'package:f_app/widgets/metric_card.dart';
import 'package:f_app/widgets/risk_gauge.dart';
import 'package:f_app/screens/detail_screen.dart';
import 'package:f_app/screens/diabetes_risk_screen.dart';
import 'package:f_app/screens/ai_chat_screen.dart';
import 'package:f_app/screens/settings_screen.dart';
import 'package:f_app/screens/food_tracker_screen.dart';
import 'package:f_app/services/report_service.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _startRealtimePolling() {
    ...
    _realtimeTimer = Timer.periodic(const Duration(seconds: 5), (timer) async {
      ...
      final dataFromFirebase = await FirebaseService.getHealthData();
      if (mounted) {
        setState(() {
          _allData = dataFromFirebase;
          ...
          if (_allData.isNotEmpty) {
            final latest = _allData.last;
            _liveGlucoseMgdl = latest.glucose;
            _liveGlucoseMmol = latest.glucose / 18.0;
            _livePulse = latest.pulse;
            _liveSpo2 = latest.spo2;
          } else {
            _liveGlucoseMgdl = null;
            _liveGlucoseMmol = null;
            _livePulse = null;
            _liveSpo2 = null;
          }
        });
  ...
  Future<void> _loadData() async {
    ...
    _allData = await FirebaseService.getHealthData();
    final now = DateTime.now();
    final todayStart = DateTime(now.year, now.month, now.day);
    _todayData = _allData
        .where((d) => d.timestamp.isAfter(todayStart))
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              if (!_s.isDeviceConnected || effectiveAllData.isEmpty) ...[
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      Text(
                        _s.isRussian
                            ? 'Отправьте данные (SpO2, глюкоза, пульс)\nв Firebase для их отображения'
                            : 'Send data (SpO2, glucose, pulse)\nto Firebase to view health metrics',
              ...
              if (_s.isDeviceConnected && effectiveAllData.isNotEmpty) ...[
                GestureDetector(
